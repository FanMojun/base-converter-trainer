...
一个纯前端的进制转换练习工具：2 / 8 / 10 / 12 / 16 进制互转、随机出题、错题本与学习数据统计。数据全部存在浏览器本地，不需要账号，断网也能用。
...
进制转换是个很小的知识点，但它属于「需要手速」的那一类：读内存布局、看位掩码、算权限位、面试在白板上推一遍，这些场景里没有余量现场从零推导。这种熟练度只能靠题量换，而现成的在线工具基本只解决了一半 —— 输入一个数、告诉你结果，然后就结束了。想练题得自己凑：开个转换器当答案对照，另开一个文档记错题，再自己数今天做了几道。

这个项目的出发点就是把上面几个动作合到一处：**转换页**随手可用，**练习页**负责出题和判题，答错自动进**错题本**，数据汇到**统计页**。四者共享同一份本地数据，不需要在工具之间来回搬运。

由此定了三条边界，代码也是按它们写的：

- **不做后端**。没有账号、排行榜和云端同步 —— 学习记录只对自己有意义，为它搭一套服务端不划算。数据放 `localStorage`，所以断网时转换、练习、错题、统计都照常可用。
- **判题只比数值**。`2D` / `2d` / `02D` 是同一个答案，不该因为大小写或前导零被判错。
- **不设位数上限**。64 位二进制串是正常输入而不是极端情况，所以中间值用 `BigInt` 而不是 `Number`（原因见「开发中实际遇到的问题」第 1 条）。

## 在线 Demo

**https://fanmojun.github.io/base-converter-trainer/**

打开即用，支持手机浏览器，也可以「添加到主屏幕」当 App 用。
...
> 并额外生成 `dist/404.html` 作为 SPA 深链接的回退页（原因见「开发中实际遇到的问题」第 3 条）。

## 项目截图

以下截图由脚本驱动真实浏览器生成（选进制、填数值、点转换，再截图），不是手摆的状态。
...
| 错题本 | 实现说明 |
| --- | --- |
| ![错题本](docs/screenshots/mistakes-data.png) | ![实现说明](docs/screenshots/algorithm.png) |
...
| 模块 | 说明 |
| --- | --- |
| **进制转换器** | 二进制 / 八进制 / 十进制 / 十二进制 / 十六进制任意互转；支持大小写混输、空格与下划线自动忽略；一键交换输入与目标进制；结果可复制 |
...
| **实现说明** | 一页纯阅读的参考材料：为什么中间值用 BigInt、转换流水线怎么走、边界输入分别是什么行为。页面上的每个数字都由实现现场算出，不写死 |
...
| **`BigInt` 做中间值** | 转换走「输入进制 → 十进制 → 目标进制」，中间那个十进制值可能远超 `Number` 的安全整数范围。`BigInt` 让任意长度输入都精确，代价是它不能与 `Number` 混算、也不能被 `JSON.stringify` 序列化 —— 所以它只活在 `utils/converter.js` 内部，`convert()` 对外返回字符串 |
| **`localStorage` 而不是后端** | 数据是「这台设备上的练习记录」，没有跨设备需求。换成后端就要处理账号、鉴权、同步冲突，还会把「断网能用」这条丢掉。代价是容量有限、可能被手动改坏，因此读取时统一过一遍结构校验（见设计要点 4） |
...
下面三处是开发过程中真正费了工夫的地方。共同点是：它们都不是「代码写错了」，而是对边界的判断错了 —— 代码跑得通，结果不对，或者换个部署位置就不对。

### 1. 大数精度：用 `Number` 算出来的十六进制会多一位

转换走「输入进制 → 十进制 → 目标进制」，十进制那一步最初打算用 `Number`。64 位二进制串是最直接的检验用例：

```js
const bin = '1'.repeat(64); // 64 位全 1

BigInt('0b' + bin).toString();    // 18446744073709551615
Number('0b' + bin).toString();    // 18446744073709552000   ← 已经错了

BigInt('0b' + bin).toString(16);  // ffffffffffffffff      （16 位）
Number('0b' + bin).toString(16);  // 10000000000000000     （17 位）
```

`Number` 的安全整数上限是 2⁵³-1（`9007199254740991`）。只看十进制，错的只是末尾几位，很容易被当成显示精度问题；但转成十六进制之后**位数都变了** —— 正确结果是 16 个 `f`，`Number` 交出的是 17 位的 `10000000000000000`。一次进位把整串推满，看上去仍像一个合法答案，没有任何异常抛出来。这类「安静地错」比崩溃难发现得多。

所以中间值改成 `BigInt`。代价是它不能与 `Number` 混算、也不能被 `JSON.stringify` 序列化，因此它只出现在 `utils/converter.js` 内部：`parseToDecimal` 从 `0n` 起步，保证整条累加链始终留在 BigInt 域内；`convert()` 对外返回的是字符串。这一条有专门的测试（64 位输入转十进制与十六进制），实现说明页里也是现场算出来给你看的。

### 2. 进制字符：校验与解析必须共用同一套归一化

「忽略空格下划线、大小写等价」听起来像展示层的体贴，其实是解析层的正确性问题。`ff 2a` 这种输入如果不先归一化，会被判成含非法字符；而一旦判错，用户连原因都看不到。

所以输入在校验之前统一过 `normalizeInput()`，它一次做三件事：去掉 `[\s_]`、转成大写、把 `null` / `undefined` 收敛成空串。校验（`validateInput` → `findInvalidCharacters`）与解析（`parseToDecimal`）都建立在归一化结果上，两边不会出现「校验放行、解析失败」这种自相矛盾。

十二进制是另一处容易漏掉的：它的字符集是 `0123456789AB`，比十六进制少四个字母。`C` 在十六进制里合法、在十二进制里非法，因此字符集必须按进制查表（`DIGIT_SETS`），不能简单写成「`A`–`F` 都算」。这一点在测试里单独覆盖了。

最后落到提示上：错误信息要指出是**哪个**字符不合法、以及**合法字符集是什么**（`“2G” 不是合法的 16 进制字符。合法字符：0123456789ABCDEF`），而不是一句「输入无效」。

### 3. 部署到 GitHub Pages 之后，子路径与深链接要分别处理

线上站点挂在 `https://fanmojun.github.io/base-converter-trainer/` 这样的子路径下，而本地开发是根路径。构建产物里的资源引用如果指向 `/assets/...`，部署后整站 404。做法是把 `base` 交给环境变量：CI 注入 `/<repo>/`，本地不传就用根路径，一份配置同时适配两种部署。

第二个坑是深链接。Pages 是纯静态托管，没有 history fallback，直接访问或刷新 `/practice` 只会拿到 Pages 的 404 页。解法是构建后把 `index.html` 复制一份成 `404.html`，由它接管未知路径，前端路由再解析出真实页面。

同理，Manifest 与 Service Worker 内部一律使用相对路径（`new URL('./', self.location)`），换部署位置不需要改任何一行 —— 这也是它能同时在根路径和子路径下正常注册的原因。

还有一处是拆包与预缓存互相冲突，过程记在下面「离线反而更难了」一节。
...
提交按阶段推进，每条对应一个具体问题。下面把每一轮的驱动原因一并记下来，因为「为什么改」比「改了哪些文件」更值得留档。
...
这一轮不加功能，做的是三件事：把界面文案里有营销腔的部分换成准确描述、给说明性内容另立一族版式、把实现上的取舍摊开写清楚。
...
fix: stop reporting the conversion record count as the total  # 记录列表有 50 条上限，超过之后
                                                              # 「总转换次数」就永远停在 50
refactor: remove duplicated rules and split the notes page out  # 「正确率」有 4 份独立实现，
                                                                # 数字分组规则抄了 3 处；顺带
                                                                # 修好懒加载页面的离线预缓存
docs: rewrite the README around problems rather than features    # 补齐项目背景与技术选型，
                                                                 # 并记录上面那三处坑
```
